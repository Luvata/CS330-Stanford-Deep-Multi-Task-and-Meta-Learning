...
if __name__ == '__main__':
    # VISUALIZE Data Loader and check order
    N = 10
    K = 1
    B = 3
    generator = DataGenerator(num_classes=N,
                              num_samples_per_class=K*2,
                              num_meta_test_classes=N,
                              num_meta_test_samples_per_class=K*2)
    all_imgs, all_labels = generator.sample_batch("train", 3)
    ...
    for batch_images, batch_labels in zip(all_imgs, all_labels):
        ...
        for i in range(N*K):
            ...
            plt.axis('off')
            plt.title(f"Train {train_labels[i]}")

            plt.subplot(K*2, N, i + 1 + N*K)  # Test index subplot start from N*K + 1
            plt.imshow(test_imgs[i])
            plt.axis('off')
            plt.title(f"Test {str(test_labels[i])}")
